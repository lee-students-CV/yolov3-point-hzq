...
train_image_folder = "D:\HZQ\jtrafficsigns\yolov3-point\data\JPEGImages/"
train_annot_folder = "D:\HZQ\jtrafficsigns\yolov3-point\data\Annotations/"
LABELS = ["warning",
          "prohibitory",
          "mandatory"]
...
ax.set_title("The total number of objects = {} in {} images".format(
    np.sum(list(seen_train_labels.values())), 15712
))
plt.show()
wh = []
...
plt.xlabel("normalized width", fontsize=15)
plt.ylabel("normalized height", fontsize=15)
...
def plot_cluster_result(plt, clusters, nearest_clusters, WithinClusterSumDist, wh):
    ...
    plt.legend(title="Mean IoU = {:5.4f}".format(WithinClusterSumDist),title_fontsize=7)
...
plt.title("Change in average accuracy",fontsize=15)
plt.ylabel("mean IOU",fontsize=15)
plt.xlabel("N clusters (= N anchor boxes)",fontsize=15)
plt.xticks(fontsize=13)
plt.yticks(fontsize=13)
plt.show()
