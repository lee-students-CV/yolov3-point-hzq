...
class kMean_parse:
    def __init__(self, n_clusters, path_txt):
        ...
        self.path = path_txt
        self.km = KMeans(n_clusters=self.n_clusters, init="k-means++", n_init=10, max_iter=3000000, tol=1e-3,
                         random_state=0)
        self._load_data()
    ...
    def plot_data(self):
        ...
        for i in range(self.n_clusters):
            plt.scatter(self.data[self.y_k == i, 0], self.data[self.y_k == i, 1], s=50, c=cValue[i % len(cValue)],
                        marker="o",
                        label="cluster " + str(i))
...
@click.command()
@click.option('--xml_path', default='D:\HZQ\jtrafficsigns\yolov3-point\data\Annotations', help='path of xml label')
